...
class CertErrorParams2Der : public CertErrorParams {
 ...
 private:
  static void AppendDer(const char* name,
                        ...
                        std::string* out) {
    *out += name;
    *out += ": " + gurl_base::HexEncode(der.data(), der.size());
  }
...
class CertErrorParams1SizeT : public CertErrorParams {
 public:
  ...
  std::string ToDebugString() const override {
    return name_ + std::string(": ") + gurl_base::NumberToString(value_);
  }
...
class CertErrorParams2SizeT : public CertErrorParams {
 public:
  ...
  std::string ToDebugString() const override {
    return name1_ + std::string(": ") + gurl_base::NumberToString(value1_) + "\n" +
           name2_ + std::string(": ") + gurl_base::NumberToString(value2_);
  }
