...
#ifndef BVC_HTTP_PARSER_HTTP_RESPONSE_HEADERS_H_
#define BVC_HTTP_PARSER_HTTP_RESPONSE_HEADERS_H_
#include <stddef.h>
#include <stdint.h>
#include <string>
...
#include "http_parser/http_version.h"
...
class HttpResponseHeaders {
 public:
  ...
  struct FreshnessLifetimes {
    FreshnessLifetimes() : freshness(quic::QuicTime::Delta::Zero()), staleness(quic::QuicTime::Delta::Zero()) {}
    // How long the resource will be fresh for.
    //base::TimeDelta freshness;
    quic::QuicTime::Delta freshness;
  ...
  static std::shared_ptr<HttpResponseHeaders> TryToCreate(
  ...
  void RemoveHeader(gurl_base::StringPiece name);
  ...
  void AddHeader(gurl_base::StringPiece name, gurl_base::StringPiece value);
  ...
  // be SetHeader().
  void SetHeader(gurl_base::StringPiece name, gurl_base::StringPiece value);
  ...
  bool GetNormalizedHeader(gurl_base::StringPiece name, std::string* value) const;
  ...
  bool HasHeaderValue(gurl_base::StringPiece name, gurl_base::StringPiece value) const;
  
  // Returns true if the response contains the specified header.
  // The name is compared case insensitively.
  bool HasHeader(gurl_base::StringPiece name) const;
  ...
  ValidationType RequiresValidation(const quic::QuicTime& request_time,
                                    const quic::QuicTime& response_time,
                                    const quic::QuicTime& current_time) const;
  ...
  FreshnessLifetimes GetFreshnessLifetimes(
      const quic::QuicTime& response_time) const;
  ...
  quic::QuicTime::Delta GetCurrentAge(const quic::QuicTime& request_time,
                                const quic::QuicTime& response_time,
                                const quic::QuicTime& current_time) const;
  ...
  bool GetMaxAgeValue(quic::QuicTime::Delta* value) const;
  bool GetAgeValue(quic::QuicTime::Delta* value) const;
  bool GetDateValue(quic::QuicTime* value) const;
  bool GetLastModifiedValue(quic::QuicTime* value) const;
  bool GetExpiresValue(quic::QuicTime* value) const;
  bool GetStaleWhileRevalidateValue(quic::QuicTime::Delta* value) const;
  ...
  bool GetTimeValuedHeader(const std::string& name, quic::QuicTime* result) const;
  ...
  static bool IsCookieResponseHeader(gurl_base::StringPiece name);
 ...
 private:
  ...
  size_t FindHeader(size_t from, gurl_base::StringPiece name) const;
  ...
  // and return true.
  bool GetCacheControlDirective(gurl_base::StringPiece directive,
                                quic::QuicTime::Delta* result) const;
...
#endif  // BVC_HTTP_PARSER_HTTP_RESPONSE_HEADERS_H_
